...
client = MongoClient("mongodb+srv://nagaveena:ChatappDB$@chatapp.7wh06.mongodb.net/<dbname>?retryWrites=true&w=majority")
...
def get_Allmessages(page=0):
    offset = page * MESSAGE_FETCH_LIMIT
    messages = list(
        messages_collection.sort('_id', DESCENDING).limit(MESSAGE_FETCH_LIMIT).skip(offset))
    for message in messages:
        message['created_at'] = message['created_at'].strftime("%d %b, %H:%M")
    return messages[::-1]
